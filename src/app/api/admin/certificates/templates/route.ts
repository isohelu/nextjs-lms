...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, message: 'Failed to retrieve certificate templates' }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO certificate_templates (name, type, template_data, logo_path, is_active, created_at, updated_at)
