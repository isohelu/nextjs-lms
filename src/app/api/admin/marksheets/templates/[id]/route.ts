...
export async function PUT(
  ...
  try {
    ...
    if (fields.length > 0) {
      ...
      db.prepare(`UPDATE marksheet_templates SET ${fields.join(', ')} WHERE id = ?`).run(...values)
    }
    ...
    return NextResponse.json({ success: false, message: 'Failed to update marksheet template' }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    db.prepare('DELETE FROM marksheet_templates WHERE id = ?').run(templateId)
    return NextResponse.json({
      success: true,
