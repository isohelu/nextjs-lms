...
export default function CertificateTemplatesPage() {
  ...
  const [titleText, setTitleText] = useState('Certificate of Achievement')
  ...
  const handleActivate = async (id: number) => {
    try {
      ...
      console.error('Error activating certificate template:', err)
    }
  }
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-10">
            ...
            <section className="space-y-4">
              <div className="flex items-center gap-2">
                <FileCheck className="h-5 w-5 text-[#007867]" />
                <h2 className="text-lg font-bold text-foreground">Course Certificate Templates</h2>
              ...
              {courseTemplates.length === 0 ? (
                <Card className="p-10 text-center border-dashed rounded-2xl bg-muted/20">
                  ...
                  <h3 className="font-semibold text-base mb-1">No course certificate templates</h3>
                  <p className="text-xs text-muted-foreground mb-4">Create your first custom course template.</p>
                  <Button size="sm" onClick={() => { setType('course'); setCreateOpen(true); }} variant="outline">
                    <Plus className="h-3.5 w-3.5 mr-1.5" /> Create Course Template
                  </Button>
                </Card>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {courseTemplates.map((template) => {
                    ...
                    const bg = template.template_data?.backgroundColor || '#f8fafc'
                    return (
                      <Card
                        key={template.id}
                      ...
                      >
                        {/* Preview Frame */}
                        <div
                          className="h-44 p-4 flex flex-col justify-between border-b relative"
                          style={{ backgroundColor: bg, borderColor: primary + '30' }}
                        >
                          ...
                          <div className="text-center my-auto">
                            <h4 className="font-bold text-sm" style={{ color: primary }}>
                              {template.template_data?.titleText || 'Certificate of Completion'}
                            </h4>
                            <p className="text-[10px] text-muted-foreground mt-0.5">Presented for Excellence</p>
                          </div>
                          <div className="flex justify-between text-[9px] text-slate-400 border-t pt-1.5 border-slate-200">
                            ...
                            <span>Seal of Verification</span>
                          </div>
                        </div>
            ...
            <section className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-foreground">Exam Certificate Templates</h2>
              ...
              {examTemplates.length === 0 ? (
                <Card className="p-10 text-center border-dashed rounded-2xl bg-muted/20">
                  <Award className="h-12 w-12 text-muted-foreground mx-auto mb-3" />
                  <h3 className="font-semibold text-base mb-1">No exam certificate templates</h3>
                  <p className="text-xs text-muted-foreground mb-4">Create your first examination credentials template.</p>
                  <Button size="sm" onClick={() => { setType('exam'); setCreateOpen(true); }} variant="outline">
                    <Plus className="h-3.5 w-3.5 mr-1.5" /> Create Exam Template
                  </Button>
                </Card>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {examTemplates.map((template) => {
                    ...
                    const bg = template.template_data?.backgroundColor || '#fdf4ff'
                    return (
                      <Card
                        key={template.id}
                        className="rounded-2xl border border-border/80 overflow-hidden bg-white shadow-xs hover:shadow-md transition-all flex flex-col"
                      >
                        ...
                        >
                          ...
                          <div className="text-center my-auto">
                            <h4 className="font-bold text-sm" style={{ color: primary }}>
                              {template.template_data?.titleText || 'Examination Certificate'}
                            </h4>
                            <p className="text-[10px] text-muted-foreground mt-0.5">High Academic Honor</p>
                          </div>
                          <div className="flex justify-between text-[9px] text-slate-400 border-t pt-1.5 border-slate-200">
                            <span>Official Examiner</span>
                            <span>Accredited Seal</span>
                          </div>
                        </div>
        ...
        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-base font-bold">Create Certificate Template</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleCreate} className="space-y-4 pt-2">
              <div>
                ...
                <Input
                  required
                  placeholder="e.g. Modern Distinction 2026"
                  value={name}
              ...
              <div>
                ...
                >
                  ...
                  <option value="exam">Examination Certificate</option>
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label className="text-xs font-semibold">Background Tint</Label>
                  <div className="flex items-center gap-2 mt-1">
                    <input
                      type="color"
              ...
              <DialogFooter className="pt-2">
                ...
                >
                  {submitting ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : 'Save Template'}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
